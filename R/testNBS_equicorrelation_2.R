...
#' result <- testNBS_equicorrelation_2(y)
#' print(result)
#' }
#'
#' @export
testNBS_equicorrelation_2 <- function(y, nivel_significancia = 0.05) {
  n <- nrow(y)
  p <- ncol(y)
  q <- p * (p + 1) / 2
  ...
  I_phi_phi <- FI[(p + 1):(p + q), (p + 1):(p + q), drop = FALSE]
  I_phi_eta <- FI[(p + 1):(p + q), (p + q + 1), drop = FALSE]
  I_eta_eta <- FI[(p + q + 1), (p + q + 1), drop = FALSE]
  I_eta_eta <- as.numeric(I_eta_eta)
  ...
  I_phi_phi_H0 <- FI_H0[(p + 1):(p + q), (p + 1):(p + q), drop = FALSE]
  I_phi_eta_H0 <- FI_H0[(p + 1):(p + q), (p + q + 1), drop = FALSE]
  I_eta_eta_H0 <- FI_H0[(p + q + 1), (p + q + 1), drop = FALSE]
  I_eta_eta_H0 <- as.numeric(I_eta_eta_H0)
  ...
  A <- restriction_matrix_equicorrelation(p)
  hh <- A %*% phi_est
